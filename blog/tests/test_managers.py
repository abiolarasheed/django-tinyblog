...
class PublishedEntryQuerySetTestCase(TestCase):
    ...
    def setUp(self):
        ...
        author, _ = get_user_model().objects.get_or_create(
            email="iamatest@gmail.com", username="iamatest"
        )

        [
            Entry.objects.get_or_create(
                title=blog_title + str(index),
    ...
    def test_entry_published(self):
        ...
        self.assertEqual(
            Entry.published.all().values("is_published").distinct().count(), 1
        )

        # Check if all results 'is_published' value is the same and is True
        self.assertTrue(
            Entry.published.all().values("is_published").distinct()[0]["is_published"]
        )
